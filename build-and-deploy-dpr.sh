#!/bin/bash
# Сборка и развёртывание приложения DPR (карта сведений о результатах рассмотрения).
# URL: http://localhost:PORT/dpr_card/
# Использование: ./build-and-deploy-dpr.sh
#
# Переменные: TOMCAT_HOME, JAVA_HOME (по умолчанию /opt/tomcat8, $TOMCAT_HOME/java)
# HOT_DEPLOY=1 по умолчанию — только WAR; полный рестарт: HOT_DEPLOY=0 ./build-and-deploy-dpr.sh
...
echo "========================================"
echo " Build and Deploy DPR ($APP_NAME)"
echo "========================================"
...
echo "[1/4] Building DPR frontend (base: /dpr_card/)..."
...
if [ ! -d "dist" ] || [ ! -f "dist/index.html" ]; then
    echo "[ERROR] DPR build failed or dist missing"
    exit 1
fi
echo "[OK] DPR frontend built"
echo ""
...
echo "========================================"
echo " DPR DEPLOYMENT COMPLETE"
echo "========================================"
echo ""
echo "  http://localhost:$TOMCAT_PORT/dpr_card/"
echo ""
