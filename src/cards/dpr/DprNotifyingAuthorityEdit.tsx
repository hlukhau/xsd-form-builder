...
export interface DprNotifyingAuthorityValue {
  country: string
  /** UID из справочника AUTHORITY (для выбора в списке; в XML csdo:AuthorityId не выводится). */
  authorityUid?: string
  name: string
  shortName: string
}

export interface DprNotifyingAuthorityEditProps {
  value: DprNotifyingAuthorityValue
  onChange: (next: DprNotifyingAuthorityValue) => void
  /** Страна УО — только просмотр (как на закладке «Уведомление» DPA). */
...
/**
 * Блок «Уполномоченный орган» DPR — по аналогии с закладкой «Уведомление» DPA:
 * идентификатор — прочерк; выбор УО из справочника; наименования только из списка.
 */
export function DprNotifyingAuthorityEdit({
  value,
  onChange,
  countryDisplay,
  isDraft,
  allowedAuthorityIds,
}: DprNotifyingAuthorityEditProps) {
