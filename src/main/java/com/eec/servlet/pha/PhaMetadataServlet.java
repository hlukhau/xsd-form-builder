...
public class PhaMetadataServlet extends HttpServlet {
    ...
    private static final String SQL_PHA =
            "SELECT p.PHAID, p.INCIDENTID, p.PHAVERSION, p.ALERTCOUNTRYID, p.CREATIONDATETIME, p.MODIFICATIONDATETIME, p.PHASTATUSID, p.DATASOURCEKINDCODE, p.ENDDATE, "
                    ...
                    + "LEFT JOIN AUTHORITY auth ON p.AUTHORITYID = auth.AUTHORITYID "
                    + "WHERE p.PHAID = ?";
        ...
        try (PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                ...
                try {
                    ...
                    if (authorityUid != null) authorityUid = authorityUid.trim();
                    if (authorityUid != null && authorityUid.isEmpty()) authorityUid = null;
                } catch (SQLException ignored) {
                ...
                if (authorityUid != null && !authorityUid.isEmpty()) {
                    json.append(",\"authorityUid\":\"").append(escapeJson(authorityUid)).append("\"");
                }
