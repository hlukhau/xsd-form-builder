...
/**
 * API доступа к карте по PPVID: подразделения (TB_DEP + TB_DEPKIND DEPKINDCODE), связь PPVDEPPERMIS(PPVID, DEPID).
 * GET /api/ppv/access?dpaid=...&source=incoming|outgoing|eec&creatorDepId=... — список { id, name, depKindCode }; при пустом списке и source=outgoing и creatorDepId — по умолчанию ЦГЭ создателя карты (из JSON прав department.depid).
 * POST /api/ppv/access — тело JSON { "dpaid": "...", "depId": "..." } — добавить подразделение (проверка права по источнику карты).
 * DELETE /api/ppv/access?dpaid=...&depId=... — удалить (проверка права по источнику карты).
 */
public class PpvAccessServlet extends HttpServlet {

    private static final String SQL_LIST = ""
            + "SELECT d.DEPID, d.DEPNAME, dk.DEPKINDCODE "
            + "FROM PPVDEPPERMIS dp "
    ...
    private static final String SQL_SOURCE = "SELECT t.DATASOURCEKINDNAME FROM VW_PPV vw LEFT JOIN DATASOURCEKIND t ON vw.DATASOURCEKINDCODE = t.DATASOURCEKINDCODE WHERE vw.PPVID = ?";
    ...
    /** Определяет право по источнику карты (VW_PPV + DATASOURCEKIND). */
