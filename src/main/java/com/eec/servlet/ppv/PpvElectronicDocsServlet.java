...
/**
 * Сведения об электронных документах и записи общего ресурса по PPVID.
 * GET /api/ppv/electronic-docs/{PPVID}
 * Источники: PPV + PPV2EDOCLINK + EDOC, CONTENTBODY из VW_PACKAGEMESSAGE (XML: EDocHeader, ResourceItemStatusDetails).
 */
public class PpvElectronicDocsServlet extends HttpServlet {

    private static final String SQL = ""
            + "SELECT dc.INFENVELOPECODE, dc.EDOCCODE, dc.EDOCID, dc.EDOCDATETIME, dc.LANGUAGECODE, dc.EDOCREFID, ms.CONTENTBODY "
            + "FROM PPV dp "
            + "JOIN PPV2EDOCLINK dpe ON dpe.PPVID = dp.PPVID "
    ...
    public void init() throws ServletException {
        super.init();
        System.out.println("[PpvElectronicDocsServlet] Initialized");
    }
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (pathInfo == null || pathInfo.isEmpty() || "/".equals(pathInfo)) {
            sendJsonError(response, HttpServletResponse.SC_BAD_REQUEST, "Укажите PPVID: /api/ppv/electronic-docs/{PPVID}");
            return;
        }
        ...
        try {
            ...
            try {
                ...
                if (msg.contains("ORA-00942")) {
                    ...
                    System.err.println("[PpvElectronicDocsServlet] VW_PACKAGEMESSAGE недоступна, используем fallback без CONTENTBODY. PPVID=" + dpaidStr);
            ...
            out.write("]");
            out.flush();
            System.out.println("[PpvElectronicDocsServlet] Served " + items.size() + " doc(s) for PPVID: " + dpaidStr);

        } catch (SQLException e) {
            System.err.println("[PpvElectronicDocsServlet] DB error for PPVID " + dpaidStr + ": " + e.getMessage());
            e.printStackTrace();
