...
public class PpvStatusChangeServlet extends HttpServlet {
    ...
    private void handleOutgoing(HttpServletResponse response, Connection conn,
        ...
        if ("mark_ready".equals(action)) {
            ...
            if (depKindCode == null || depKindCode.trim().isEmpty()) {
                log("[PpvStatusChange] mark_ready: depKindCode still null; guid=" + guid + ", mapSize=" + RightsJsonStore.guidMap.size() + ", mapContainsGuid=" + (guid != null && RightsJsonStore.guidMap.containsKey(guid)));
    ...
    private static String resolveDepKindCodeFromRights(Connection conn, String guid) throws SQLException {
        ...
        if (rightsJson == null || rightsJson.isEmpty()) {
            System.out.println("[PpvStatusChange] resolveDepKindCodeFromRights: no rights for guid=" + g + ", mapSize=" + RightsJsonStore.guidMap.size() + ", keys=" + RightsJsonStore.guidMap.keySet());
            return null;
        }
