...
/** Пустая строка документа для добавления в форме SMR. */
export function emptySmrResultDocRow(): SmrResultDocRow {
  return {
    countryCode: '',
    languageCode: 'ru',
...
export function smrResultDocRowToMeasureDocDetails(row: SmrResultDocRow): MeasureDocDetails {
...
export function measureDocDetailsToSmrResultDocRow(doc: MeasureDocDetails): SmrResultDocRow {
  return {
...
export function hasSmrResultDocRowContent(row: SmrResultDocRow): boolean {
  if (
    s(row.countryCode) ||
    s(row.languageCode) ||
    s(row.docKindCode) ||
